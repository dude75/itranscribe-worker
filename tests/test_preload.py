...
def test_preload_sets_writable_numba_and_mpl_caches(
    ...
    assert os.environ["NUMBA_CACHE_DIR"] == str((tmp_path / "data" / "numba_cache").resolve())
...
def test_preload_accepts_comma_separated_families() -> None:
    settings = Settings(
...
@pytest.mark.parametrize(
    "field, value",
    [
        ("PRELOAD_ASR", "whisper,all"),
        ("PRELOAD_DIARIZATION", "all,nemo"),
        ("PRELOAD_ASR", ""),
        ("PRELOAD_ASR", ","),
    ],
)
def test_preload_rejects_mixed_all_and_empty(field: str, value: str) -> None:
    with pytest.raises(ValidationError):
        Settings(**{field: value}, _env_file=None)


def test_parakeet_chunk_sec_rejects_non_positive() -> None:
    with pytest.raises(ValidationError):
        Settings(PARAKEET_CHUNK_SEC=0, _env_file=None)
    with pytest.raises(ValidationError):
        Settings(PARAKEET_CHUNK_SEC=-1, _env_file=None)
...
def test_preload_comma_separated_constructs_listed_families(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    constructed: list[str] = []
    monkeypatch.setattr("app.engines.cache.FasterWhisperASR", _spy("whisper", constructed))
