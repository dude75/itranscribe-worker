...
def _transcript_fernet() -> Fernet:
    digest = hashlib.sha256(get_settings().API_TOKEN.encode()).digest()
    return Fernet(base64.urlsafe_b64encode(digest))


def _encrypt_transcript(payload: str) -> str:
    return _transcript_fernet().encrypt(payload.encode()).decode()


def _decode_transcript(raw: str | None) -> list[dict[str, Any]] | None:
    if not raw:
        return None
