...
При первом старте идёт **preload** семейств из `PRELOAD_ASR` и `PRELOAD_DIARIZATION` (по умолчанию `all` = все четыре). У каждого семейства первая копия берёт веса из `MODELS_DIR`, если они уже есть, иначе качает с Hugging Face; остальные копии `WORKERS` того же семейства читают только этот кэш (без etag в Hub). Веса пропущенных семейств не скачиваются. Сбой загрузки — `unavailable`; семейство выключено в preload — `disabled`. Процесс живой. Повторная задача не должна качать веса, если они уже лежат в `./data/models`.
